...
'''
    ...
    def _createToolBars(self):
        ...
        penToolBar.addAction(self.addAction(f'Remove drawings', self._getIcon('remove'), self.removeDrawing()))
        ...
        boardToolBar.addAction(self.addAction("Exit", self._getIcon('exit'), sys.exit))
...
def main():
    ...
    screen_idx = show_screen_selection(screens) - 1
    if screen_idx == -1:
        print('No screen chosen, exiting.')
        sys.exit(0)

    screen, screen_geom, pixmap = screens[screen_idx]
    ScreenPenWindow(screen, screen_geom, pixmap)
    sys.exit(app.exec_())
